...
Below is a table showing the list of error codes you might come across while using the TMDB API and what they mean:

| Code | HTTP Status | Message                                                                              | Possible Solution                                                              |
| -----| ----------- | -----------------------------------------------------------------------------------  | ----------------------------------------------------------------------------   |                                                              
|   1  | 200	     | Success.                                                                             | -------                                                                        |
|   2  | 501	     | Invalid service: this service does not exist.                                        | Check that the endpoint you are working with is valid                          |
|   3  | 401	     | Authentication failed: You do not have permissions to access the service.            | Confirm that your authentication credentials are valid.                        | 
|   4  | 405	     | Invalid format: This service doesn't exist in that format.                           | This happens when you access an endpoint in the wrong format.                  |
|   5  | 422	     | Invalid parameters: Your request parameters are incorrect.                           | Check the request for invalid parameters.                                      |
|   6  | 404	     | Invalid id: The pre-requisite id is invalid or not found.                            | Confirm that you are using a correct id in your request.                       |
|   7  | 401	     | Invalid API key: You must be granted a valid key.                                    | Confirm that your API key is valid.                                            |                                                               
|   8  | 403	     | Duplicate entry: The data you tried to submit already exists.                        | Check that the new data doesn't already exists.                                |                                                  
|   9  | 503	     | Service offline: This service is temporarily offline, try again later.               | The TMDB API is temporarily down. Try again later                              |                                              |
...
|   47 | 400	     | The input is not valid.                                                              | Check that your request parameters and response body have the right data type  |                                                                    |