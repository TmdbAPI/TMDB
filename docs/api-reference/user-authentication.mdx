...
The steps on how to do this are outlined in this guide. Read more about other authentication methods [here](https://emiloju.mintlify.app/api-reference/authentication).
...
The [create request token](https://emiloju.mintlify.app/api-reference/authentication/create-request-token) endpoint is used to generate a `request_token`.
...
The [create session](https://emiloju.mintlify.app/api-reference/authentication/create-session) endpoint is used to create a new session id for a user. By calling the new session method with the request token that has been approved by the user, 
...
Not all application have access to a web view so this can be used as a substitute. The [validate request token (with Login)](https://emiloju.mintlify.app/api-reference/authentication/validate-request-token-with-login) endpoint is used to do this and it requires
