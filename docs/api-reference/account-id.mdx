...
### As an application
Get your application's `api_key` and `access_token`, then send a `GET` request to [Get account details](https://emiloju.mintlify.app/api-reference/accounts/get-account-details) to retrieve your Account ID from the response object.
Below is an example:
...
2. Send a `GET` request to the [Get account details](https://emiloju.mintlify.app/api-reference/accounts/get-account-details) endpoint and add the `session_id` as a query parameter. Here is an example below:
...
The Account ID will be included in the response object as seen in the [Get account details](https://emiloju.mintlify.app/api-reference/accounts/get-account-details) endpoint. 
