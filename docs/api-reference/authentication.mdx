...
<Tabs>
     <Tab title="ALA">
          ...
          Use the `api_key` over the `access_token` only when you really need to. For example, the [Get account details](https://emiloju.mintlify.app/api-reference/accounts/get-account-details) endpoint uses an API Key. 
     ...
     <Tab title="User authentication">
          ...
          User authentication is controlled with a `session_id` query parameter. Read this [guide](https://emiloju.mintlify.app/api-reference/user-authentication) to see how to generate a `session_id`.
     ...
     <Tab title="Guest sessions">
          ...
          Creating a guest session is as simple as calling the [new guest session method](https://emiloju.mintlify.app/api-reference/authentication/create-guest-session).
...
<Steps>
     <Step title="[Create](https://www.themoviedb.org/signup) a TMDB account/[Login](https://www.themoviedb.org/login) to your TMDB dashboard">
     </Step>
