...
There are three ways to authenticate with The Movie Database (TMDB) API. They include the following:
...
The default method to authenticate is with your access token. This token is expected to be sent along as an Authorization header. 
...
The `api_key` is the second authentication method. Although, it cannot be used for all endpoints. 
Use the `api_key` over the access token only when you really need to. For example, the [endpoint]() endpoint requires an API Key. 
...
maintain their favourite and watch lists as well as do things like create and edit custom lists, all while staying in sync with their account on TMDB. User authentication is 
controlled with a `session_id` query parameter. Read this [guide]() to see how to generate a `session_id`.
...
After submitting the application, you will be presented with both your API key and Bearer token as seen below:
