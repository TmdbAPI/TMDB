---
title: Finding data
description: Learn how to find data on TMDB
icon: "database"
---
...
- `/search` - [Text based search](https://emiloju.mintlify.app/api-reference/search/) is the most common way. You provide a query string and we provide the closest match. 
...
The [discover](https://emiloju.mintlify.app/api-reference/discover/) methods make this easy.

- `/find` - The last but still very useful way to find data is with existing [external IDs](https://emiloju.mintlify.app/api-reference/find/find-by-external-id). 
...
Take a look at the [search & query](/documentation/search) page for some basic workflows you might use to search and query data.