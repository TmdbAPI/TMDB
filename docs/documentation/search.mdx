...
description: Learn how to search and query for a movie effectively.
...
Here's an example of the first item:
...
```bash Example Details Query
curl --request GET \
     --url 'https://api.themoviedb.org/3/movie/343611' \
     --header 'Authorization: Bearer ACCESS_TOKEN'
```

This will return all of the main movie details as outlined in the movie details documentation. I would also suggest taking a read through the append to response document as it
outlines how you can make multiple sub requests in one. For example, with videos:

```bash Example Append Request
curl --request GET \
